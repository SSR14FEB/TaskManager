  ...
  return (
    <div className="overflow-x-auto rounded-lg mt-3">
      <table className="min-w-full">
        ...
        <tbody>
          {tableData.map((task) => (
            <tr className="border-t border-gray-200" key={task.id}>
              <td className="my-3 mx-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden">{task?.title || ""}</td>
              <td className="px-4 py-4">
                <span
                  className={`px-2 py-1 text-xs rounded inline-block ${getStatusBadge(
                    task?.status || ""
                  )}`}
                >
                  {task?.status || ""}
              ...
              <td className="px-4 py-4 text-gray-700 text-[13px] hidden md:table-cell">
                  {moment(task.createdAt).format("dddd Do MMM YYYY")}
              </td>
            </tr>
          ))}
        </tbody>
